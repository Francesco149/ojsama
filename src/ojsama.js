const {log, array_toFixed, isUndefined} = require('./interals');
...
// represents the bpm multiplier as ```-100 * bpm_multiplier```
  ...
  std_diff_hitobject.prototype.toString = function() {
    return (
      '{ strains: [' +
      array_toFixed(this.strains, 2) +
      '], normpos: [' +
      array_toFixed(this.normpos, 2) +
      '], is_single: ' +
      this.is_single +
      ' }'
    );
  ...
  std_diff.prototype._spacing_weight = function(
      type,
      distance,
      delta_time,
      prev_distance,
